...
P = plot(layout=(3,3), size=(900, 500))
for i=1:9
    ...
    @df tf plot!(:time, :interface, linewidth=2, color=:red, subplot=i)
    annotate!((0.07, 0.9), text(strain, :gray, :left, 8), subplot=i)
    plot!(xlabel="Time [hr]", ylabel="Height [μm]", subplot=i, legend=false)
end 
plot(P, bottom_margin=3mm, left_margin=5mm, grid=false)
savefig("figs/fig5/fig5.pdf")

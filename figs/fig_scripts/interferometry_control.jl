...
append!(comparison_time, mean(tf[final_index, :].time))
##
nam = ["Aeromonas", "E. coli", "Yeast (aa)", "Yeast", "V. cholerae (wt)", "V. cholerae (EPS-)"]
ctg = repeat(["Interferometry", "Control"], inner = 6)
##
groupedbar(nam, reduce(hcat, final_heights)', err=reduce(hcat, heights_error)',
           label = ["Interferometry" "Control"], ylabel="Average Height [μm]", xrotation=40, bottom_margin=3mm, size=(500, 400))
